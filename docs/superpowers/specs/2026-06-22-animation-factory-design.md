...
- **NEXT (M2):** **content & animation vocabulary buildout (ADR-003), Tier-A-first** — ✅ `@remotion/shapes`+`@remotion/paths`+`flubber` morph (shape layer) · ✅ `effects[]` via SVG/CSS filters + motion blur (the **core-effects plugin**) · generators (✅ scatter, water, particles, fire, crowd) · ✅ `clip` (nested pre-composition — §13.3); `environment` is the follow-on (a clip used as a backdrop) · `attach`/`parts` (§8.1) · ✅ text/typography (§11.3) · color-script (§11.4) · object/prop specs · data-viz.
...
**M2 (next):** ✅ **Shading & Depth model** (§11.1 — 2026-06-22), ✅ **multi-scene storytelling + transitions (§11.2 — 2026-06-22: beat→scene sequencing on the global timeline + `@remotion/transitions` `TransitionSeries`; match-cuts/morph-match/camera-continuous still fall back to fade)**; ✅ **reusable `clip` nested composition (§13.3 — 2026-06-22: shared `defs.clips` + `clip`-layer refs + Essential-Graphics `params`/`args` + Remotion `<Sequence>` local timeline; per-instance namespacing/seeding → one clip used twice renders distinctly yet byte-identically; recursion to depth ≥ 2)**, ✅ **kinetic typography (§11.3 — 2026-06-22)**; remaining: **`environment`** (a scene-template clip = a full background+ambience bundle, i.e. a clip used as a backdrop — the clip follow-on), compositional rigs (`attach` between rigs, `parts` selection, presets), rig instancing/crowds (DragonBones factory), object detail (`scatter` generator + baking + Pixi instancing + shape budget, §10.1), more generators (water, particles), Lottie ingest, morph channel, **layer `effects[]` + motion blur**, **color-script / palette-per-beat (§11.4)**, stagger. **M3+:** full **audio + sound design** (TTS, lip-sync, captions, **narration-synced text**, SFX-from-events, mixing — P4/P7), composition **`post[]`** grade, AI asset-gen (P3), smart layout (P6/P9), LLM script-expander (P1).
