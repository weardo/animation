...
**Date:** 2026-06-22
**Status:** In build — M1 complete; **M2 complete (vocabulary fast-track, 2026-06-22)**; **M3 started — narration MVP DONE (2026-06-22)**. **§0 reconciles the spec with what's actually built** (the design evolved via `docs/factory/ADR-001`, `ADR-002`, and `docs/factory/DECISIONS.md`).
...
- **LATER (M3, remaining):** rest of audio (whisper captions + lip-sync · SFX-from-events · music bed + ducking) (P4/P7) · LLM script-expander (P1) · smart layout (P6/P9) · `post[]` grade · AI asset-gen (P3) · **Tier-B GPU enablement** (GL Transitions / Pixi filters / three / skia, gated on deterministic GPU capture) · small follow-ups (per-silhouette form-shade, far-layer atmospheric tint, OTIO export, `factory bundle`/`list`).
...
## 12. Audio, Narration & Sound Design (narration DONE 2026-06-22; rest later)

Remotion handles audio natively. The Scene IR reserves `audio[]` cues; the TTS pass now fills the narration ones (the rest — captions/SFX/music — are still later):

- ✅ **Narration MVP (DONE 2026-06-22)** — OFFLINE TTS synthesizes each beat's `say` into a **content-addressed, cached wav** at BUILD time (golden rule 2: voice never touches frames/runtime — exactly like `factory:gen` source material / the vendored font), the compile path emits a `kind:"narration"` `audio[]` cue at the beat's scene start, the compositor plays it via `<Audio>` in a `<Sequence from={cue.at}>`, and `renderMedia` muxes it (h264+aac). The TTS engine is **abstracted + swappable** (`src/cli/narrate.ts`): **espeak-ng** is the deterministic always-available DEFAULT; **Coqui XTTS v2** is wired via an isolated venv (`.venv-tts/bin/tts`, needs its own Python 3.11 env) and FALLS BACK to espeak-ng with a warning if missing — the cached wav is the deterministic artifact regardless of which engine produced it. **Determinism (golden rule 1):** the engine runs ONCE (skip-if-exists), so a re-render replays the FIXED wav → BOTH the decoded video AND audio streams are byte-identical across cold processes, even though a TTS engine may be stochastic. Compile entry: `src/cli/narrate-pass.ts` (wiring) + `src/cli/render.ts` (`--engine`/`--voice`/`--wpm`; `--no-audio` to skip). Demo: `examples/narration-demo.yaml` → `projects/narration-demo`. **Remaining (not built):** whisper word-level alignment → captions + lip-sync visemes · SFX-from-animation-events · music bed + per-frame ducking (the `<Audio volume>` hook is reserved).
