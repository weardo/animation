...
- **NEXT (M2):** **content & animation vocabulary buildout (ADR-003), Tier-A-first** — ✅ `@remotion/shapes`+`@remotion/paths`+`flubber` morph (shape layer) · ✅ `effects[]` via SVG/CSS filters + motion blur (the **core-effects plugin**) · generators (✅ scatter, water, particles, fire, crowd) · `clip`/`environment` (§13.3) · `attach`/`parts` (§8.1) · text/typography (§11.3) · color-script (§11.4) · object/prop specs · data-viz.
