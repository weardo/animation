...
- **LATER (M3, remaining):** **lip-sync** (viseme/mouth-shape from audio) · **whisper-precision captions** (real word-timestamp alignment — beyond the deterministic even-split) · **LLM script-expander** (P1; needs an engine decision) · **smart layout/camera director** (P6/P9) · `post[]` grade · AI asset-gen (P3) · **Tier-B GPU enablement** (GL Transitions / Pixi filters / three / skia, gated on deterministic GPU capture) · small follow-ups (per-silhouette form-shade, far-layer atmospheric tint).
- **ADR follow-ups:** formalize `AssetProvider`/`LibraryResolver` TS interfaces (the last open interface work).
