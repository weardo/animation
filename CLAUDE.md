...
`src/engine` (ADR-005 plugin core: extension-point registries + EngineAPI + plugin/manifest contract + loader) · `src/ir` (Zod IR) · `src/library` (registry/resolver/lockfile) · `src/factory` (data-driven asset gen: CharacterSpec → builder) · `src/project` (project bundle: manifest + scene + lock) · `src/pipeline` (passes) · `src/render` (compositor + Remotion entry + procedural/DragonBones rig providers) · `src/generators` (procedural) · `src/cli` · `plugins/<id>/` (capability = code: `plugin.json` + `index.ts register(api)`; built-ins = core-generators/core-rigs/blob-creature) · `library/` (shared asset catalog) · `projects/<id>/` (reproducible video bundles) · `examples/` (source scripts + specs) · `docs/` (spec, ADRs, factory standard).
...
**Plugin model (ADR-005, DONE):** the engine core is minimal — it owns generic **extension-point registries** + an `EngineAPI`; **capability is contributed by plugins (code)**, content stays in the **library (data)**. Live extension points: `generators`, `rigProviders`, `characterStyles` (built-ins ship as the core-generators/core-rigs/blob-creature plugins); stubs reserved: `effects`/`transitions`/`layerTypes`/`passes`. **New capability (effects, transitions, text, color, styles, IR passes) is authored AS A PLUGIN** — never a core edit; `loadPlugins()` validates each `plugin.json`, orders by `deps`, calls `register(api)` before render. A plugin obeys the golden rules and is gated by `verify-render` (cross-process byte-identical).
